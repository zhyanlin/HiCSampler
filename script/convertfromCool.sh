...
cooler  dump $input -t bins  -c 'chrom,start,weight' --na-rep 999999 --join -H | awk -v scale=$scale -v chrom=$chrom -v startbp=$startbp -v endbp=$endbp -v resol=$resol '{if($1==chrom && $2>=startbp-resol && $2<endbp-resol)print NR-1"\t"1/$NF/scale}' >  ${output}.bias.tsv
